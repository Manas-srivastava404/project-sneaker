...
function openQuickView(index) {
  ...
  const inrPrice = p.price * USD_TO_INR;

  document.getElementById("quick-img").src = p.img;
  document.getElementById("quick-title").textContent = p.name;
  document.getElementById("quick-desc").textContent = `₹${inrPrice} - Premium quality. Best comfort.`;

  const carousel = document.getElementById("quick-images");
  carousel.innerHTML = p.images
    .map(img => `<img src="${img}" onclick="document.getElementById('quick-img').src='${img}'" />`)
    .join('');
  ...
  container.innerHTML = filtered
    .map((p, i) => `
      <div class="product-card">
        ...
        <h3>${p.name}</h3>
        <p>₹${(p.price * USD_TO_INR).toLocaleString()}</p>
